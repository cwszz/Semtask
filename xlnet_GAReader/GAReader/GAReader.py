...
from Bert_GAReader.Models.BertEmbedding import BertEmb
...
class GAReader(nn.Module):
    ...
    def forward(self, batch):
        ...
        # article, article_lengths = batch[1]
        q_ids,q_seg,q_mask = batch[0],batch[1],batch[2]
        a_ids,a_seg,a_mask = batch[3],batch[4],batch[5]
        option0_ids,option0_seg,option0_mask = batch[6],batch[7],batch[8]
        option1_ids,option1_seg,option1_mask = batch[9],batch[10],batch[11]
        option2_ids,option2_seg,option2_mask = batch[12],batch[13],batch[14]
        option3_ids,option3_seg,option3_mask = batch[15],batch[16],batch[17]
        option4_ids,option4_seg,option4_mask = batch[18],batch[19],batch[20]
        ...
        question_emb = self.dropout(self.word_embedding(input_ids=q_ids,attention_mask=q_mask,token_type_ids=q_seg,max_l=128))
        # question_emb: [question_len, batch_size, emd_dim]

        option0_emb = self.dropout(self.word_embedding(input_ids=option0_ids,attention_mask=option0_mask,token_type_ids=option0_seg))
        option1_emb = self.dropout(self.word_embedding(input_ids=option1_ids,attention_mask=option1_mask,token_type_ids=option1_seg))
        option2_emb = self.dropout(self.word_embedding(input_ids=option2_ids,attention_mask=option2_mask,token_type_ids=option2_seg))
        option3_emb = self.dropout(self.word_embedding(input_ids=option3_ids,attention_mask=option3_mask,token_type_ids=option3_seg))
        option4_emb = self.dropout(self.word_embedding(input_ids=option4_ids,attention_mask=option4_mask,token_type_ids=option4_seg))
        ...
        ei = torch.ones(question_emb.size(0)).long().to(question_emb.device)
        question_hidden, question_out = self.rnn(question_emb,question_emb.size(1)*ei)
        # question_out: [batch_size, question_len, hidden_size * 2]
        # question_hidden: [batch_size, hidden_size * 2]

        option0_hidden, option0_out = self.rnn(option0_emb, option0_emb.size(1)*ei)
        option1_hidden, option1_out = self.rnn(option1_emb, option1_emb.size(1)*ei)
        option2_hidden, option2_out = self.rnn(option2_emb, option2_emb.size(1)*ei)
        option3_hidden, option3_out = self.rnn(option3_emb, option3_emb.size(1)*ei)
        option4_hidden, option4_out = self.rnn(option4_emb, option4_emb.size(1)*ei)
        # option_out: [batch_size, option_len,  hidden_size * 2]

        _, article_out = self.rnn(article_emb, article_emb.size(1)*ei)
        ...
        for layer in range(self.ga_layers):
            ...
            _, article_out = self.ga_rnn(article_emb, article_emb.size(1)*ei)
